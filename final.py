...
REPORT_PDF_PATH = Path("march_madness_report.pdf")
COEF_CHART_PATH = Path("upset_model_coefficients.png")

# Human-readable y-axis labels for the coefficient chart
...
def feature_chart_label(feature: str):
    if feature in FEATURE_CHART_LABELS:
        return FEATURE_CHART_LABELS[feature]
    return feature.replace("_", " ")

# Create the main chart (coefficient horizontal bar chart)
def save_coefficient_chart(coef_df: pd.DataFrame, path: Path):
    ...
    ax.set_yticks(y)
    labels = [feature_chart_label(f) for f in df["Feature"]]
...
# Put all team names in same format
...
# Read all per-year CBB conference game CSVs
def load_cbb_data():
...
def build_h2h_lookup(cbb: pd.DataFrame, all_tournament_teams: set):
    # Build a dict: (season, teamA_kenpom, teamB_kenpom) -> list of game dicts
    # Each game dict contains: home_team, away_team, home_score, away_score, winner, margin
    if cbb.empty:
        return {}
    ...
    for kp_name in sorted(all_tournament_teams):
        if kp_name in _KENPOM_TO_CBB:
            ...
            continue
        norm = normalize_team_name(kp_name)
...
def compute_h2h_features(year: int, fav_team: str, dog_team: str, lookup: dict):
...
# --------------------------------------------------
def build_games_from_matchups(matchups: pd.DataFrame):
    df = matchups.copy()
...
# --------------------------------------------------
def prep_team_features(kenpom: pd.DataFrame):
    feature_cols = [
...
# --------------------------------------------------
def merge_team_features(games_df: pd.DataFrame, team_features: pd.DataFrame):
    rename_template = {
...
# --------------------------------------------------
def add_difference_features(df: pd.DataFrame):
    df = df.copy()
    diff_pairs = [
...
# --------------------------------------------------
# 6b. ADD HEAD-TO-HEAD REGULAR-SEASON FEATURES
# --------------------------------------------------
def add_h2h_features(games_df: pd.DataFrame, lookup: dict):
...
def main():
    ...
    # --------------------------------------------------
    # LOAD CBB DATA AND ADD HEAD-TO-HEAD FEATURES
    # --------------------------------------------------
